...
class Consultare_Customizer_Utilities {
	/**
	...
	public static function register_option( $option ) {
		...
		$defaults = apply_filters( 'consultare_customizer_defaults', array() );
		...
		if ( ! empty( $get_posts ) ) {
			foreach ( $get_posts as $get_post ) {
				...
				if ( empty( $title ) ) {
					...
					$title = sprintf( esc_html__( 'ID: %s No Title', 'consultare' ), $get_post->ID );
				}
	...
	static function section_visibility() {
		$options = array(
			'disabled'    => esc_html__( 'Disabled', 'consultare' ),
			'homepage'    => esc_html__( 'Homepage / Frontpage', 'consultare' ),
			'entire-site' => esc_html__( 'Entire Site', 'consultare' ),
		);

		return apply_filters( 'consultare_section_visibility_options', $options );
	}
}
 ...
 * Initialize class
 */
$consultare_utilities = new Consultare_Customizer_Utilities();
