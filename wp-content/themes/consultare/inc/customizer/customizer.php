...
class Consultare_Customizer {
	...
	public function register_options( $wp_customize ) {
		...
		$section_visibility = Consultare_Customizer_Utilities::section_visibility();

		$section_visibility['excluding-home'] = esc_html__( 'Excluding Homepage', 'consultare' );

		Consultare_Customizer_Utilities::register_option(
			array(
				'settings'          => 'consultare_header_image_visibility',
				'type'              => 'select',
		...
		$wp_customize->add_section( new Consultare_Upsell_Section( $wp_customize, 'upsell_section',
			array(
				'title'           => esc_html__( 'Consultare Pro Available', 'consultare' ),
				'url'             => 'https://fireflythemes.com/themes/consultare-pro',
	...
	public function preview_js() {
		...
		wp_enqueue_script( 'consultare-customizer', get_template_directory_uri() . '/js/customizer-preview' . $min . '.js', array( 'customize-preview' ), consultare_get_file_mod_date( '/js/customizer-preview' . $min . '.js' ), true );
	}

	/**
	 * Binds the JS listener to make Customizer consultare_color_scheme control.
	 *
	 * @since 1.0
	 */
	public function customize_control_js() {
		...
		// Enqueue Select2.
		wp_enqueue_script( 'consultare-select2-js', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/select2' . $min . '.js', array( 'jquery' ), '4.0.13', true );
		wp_enqueue_style( 'consultare-select2-css', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'css/select2' . $min . '.css', array(), '4.0.13', 'all' );
		
		// Enqueue Custom JS and CSS.
		wp_enqueue_script( 'consultare-custom-controls-js', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/customizer' . $min . '.js', array( 'jquery', 'jquery-ui-core', 'consultare-select2-js' ), consultare_get_file_mod_date( '/js/customizer' . $min . '.js' ), true );
		
		wp_enqueue_style( 'consultare-custom-controls-css', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'css/customizer' . $min . '.css', null, consultare_get_file_mod_date( '/css/customizer' . $min . '.css' ), 'all' );
 ...
 */
$consultare_customizer = new Consultare_Customizer();
